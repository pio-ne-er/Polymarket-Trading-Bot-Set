...
pub struct TradingConfig {
    ...
    /// Trailing bot: first token trigger = lowest + this. Overrides trailing_stop_point for first buy. In config.json: "trailing_stop_point_first": 0.03
    pub trailing_stop_point_first: Option<f64>,
    /// Trailing bot: second token (opposite) trigger = opposite_lowest + this. Overrides trailing_stop_point for second buy. In config.json: "trailing_stop_point_second": 0.03
    pub trailing_stop_point_second: Option<f64>,
    ...
    /// Trailing bot: market period in minutes. Use 15 for 15-minute markets (e.g. btc-updown-15m-*), 5 for 5-minute markets (e.g. btc-updown-5m-*). Default: 15.
    pub trailing_market_minutes: Option<u64>,
    /// Trailing bot: when true (default), buy opposite token when price >= (1 - first_bought + 0.10) (stop loss). When false, only the trailing-stop trigger is used for the second buy.
    pub trailing_stop_loss_enabled: Option<bool>,
    /// Trailing bot: when true (default), use CLOB market WebSocket for prices. When false, use REST polling.
    pub trailing_use_websocket: Option<bool>,
    /// Trailing bot: second-token first hedging window in seconds (e.g. 90 for 1m30s). If set, overrides market default (60 for 5m, 120 for 15m).
    pub trailing_first_window_seconds: Option<u64>,
    /// Trailing bot: second-token second hedging window in seconds (e.g. 180 for 3min). If set, overrides market default (150 for 5m, 240 for 15m).
    pub trailing_second_window_seconds: Option<u64>,
...
impl Default for TradingConfig {
    fn default() -> Self {
        Self {
            ...
            trailing_stop_point_first: None,
            trailing_stop_point_second: None,
            dual_limit_hedge_trailing_stop: Some(0.03),
            trailing_shares: Some(10.0),
            trailing_market_minutes: Some(15),
            trailing_stop_loss_enabled: Some(true),
            trailing_use_websocket: Some(true),
            trailing_first_window_seconds: None,
            trailing_second_window_seconds: None,
