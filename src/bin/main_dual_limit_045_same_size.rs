...
fn run_trailing_stop_replay(
    snapshots: &[polymarket_arbitrage_bot::backtest::PriceSnapshot],
...
async fn main() -> Result<()> {
    ...
    if args.is_history_replay() {
        ...
        let snapshots = polymarket_arbitrage_bot::backtest::load_price_history(&to_load)
        ...
        async move {
            ...
            if is_simulation {
                if let Some(tracker) = trader.get_simulation_tracker() {
                    ...
                    for trade in &pending_limits {
                        let token_price = match &trade.token_type {
                            crate::detector::TokenType::BtcUp => snapshot.btc_market.up_token.as_ref(),
                            crate::detector::TokenType::BtcDown => snapshot.btc_market.down_token.as_ref(),
                            crate::detector::TokenType::EthUp if enable_eth => snapshot.eth_market.up_token.as_ref(),
                            crate::detector::TokenType::EthDown if enable_eth => snapshot.eth_market.down_token.as_ref(),
                            crate::detector::TokenType::SolanaUp if enable_solana => snapshot.solana_market.up_token.as_ref(),
                            crate::detector::TokenType::SolanaDown if enable_solana => snapshot.solana_market.down_token.as_ref(),
                            crate::detector::TokenType::XrpUp if enable_xrp => snapshot.xrp_market.up_token.as_ref(),
                            crate::detector::TokenType::XrpDown if enable_xrp => snapshot.xrp_market.down_token.as_ref(),
                            _ => None,
                        };
                ...
                for (market_name, condition_id, up_opt, down_opt, up_type, down_type) in [
                    ("BTC", &snapshot.btc_market.condition_id, snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref(), crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown),
                    ("ETH", &snapshot.eth_market.condition_id, snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref(), crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown),
                    ("SOL", &snapshot.solana_market.condition_id, snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref(), crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown),
                    ("XRP", &snapshot.xrp_market.condition_id, snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref(), crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown),
                ] {
            ...
            if is_simulation && (time_elapsed_seconds % 15 == 0 || time_elapsed_seconds <= 30) {
                ...
                let hedge_trailing_stop = config.trading.dual_limit_hedge_trailing_stop.unwrap_or(0.03);
                let mut sim_markets: Vec<(&str, &str, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                if enable_btc {
                    if let (Some(u), Some(d)) = (snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref()) {
                        sim_markets.push(("BTC", &snapshot.btc_market.condition_id, u, d, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                    }
                }
                if enable_eth {
                    if let (Some(u), Some(d)) = (snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref()) {
                        sim_markets.push(("ETH", &snapshot.eth_market.condition_id, u, d, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                    }
                }
                if enable_solana {
                    if let (Some(u), Some(d)) = (snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref()) {
                        sim_markets.push(("SOL", &snapshot.solana_market.condition_id, u, d, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                    }
                }
                if enable_xrp {
                    if let (Some(u), Some(d)) = (snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref()) {
                        sim_markets.push(("XRP", &snapshot.xrp_market.condition_id, u, d, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                    }
                }
            ...
            if !HEDGE_DISABLED {
                ...
                let mut markets_to_cancel: Vec<(&str, &str, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                if enable_btc {
                    if let (Some(btc_up), Some(btc_down)) = (snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref()) {
                        ...
                        if !cancelled_guard.contains(&key) && !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_cancel.push(("BTC", &snapshot.btc_market.condition_id, btc_up, btc_down, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                        }
                    }
                }
                if enable_eth {
                    if let (Some(eu), Some(ed)) = (snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref()) {
                        ...
                        if !cancelled_guard.contains(&key) && !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_cancel.push(("ETH", &snapshot.eth_market.condition_id, eu, ed, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                        }
                    }
                }
                if enable_solana {
                    if let (Some(su), Some(sd)) = (snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref()) {
                        ...
                        if !cancelled_guard.contains(&key) && !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_cancel.push(("SOL", &snapshot.solana_market.condition_id, su, sd, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                        }
                    }
                }
                if enable_xrp {
                    if let (Some(xu), Some(xd)) = (snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref()) {
                        ...
                        if !cancelled_guard.contains(&key) && !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_cancel.push(("XRP", &snapshot.xrp_market.condition_id, xu, xd, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                        }
                    }
                }
            ...
            if !HEDGE_DISABLED {
                let hedge_guard = hedge_executed_for_market.lock().await;
                let two_min_guard = two_min_hedge_markets.lock().await;
                let mut markets_to_check: Vec<(&str, &str, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                if enable_btc {
                    if let (Some(btc_up), Some(btc_down)) = (snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref()) {
                        let key = (snapshot.period_timestamp, snapshot.btc_market.condition_id.clone());
                        if !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_check.push(("BTC", &snapshot.btc_market.condition_id, btc_up, btc_down, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                        }
                    }
                }
                if enable_eth {
                    if let (Some(eu), Some(ed)) = (snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref()) {
                        let key = (snapshot.period_timestamp, snapshot.eth_market.condition_id.clone());
                        if !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_check.push(("ETH", &snapshot.eth_market.condition_id, eu, ed, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                        }
                    }
                }
                if enable_solana {
                    if let (Some(su), Some(sd)) = (snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref()) {
                        let key = (snapshot.period_timestamp, snapshot.solana_market.condition_id.clone());
                        if !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_check.push(("SOL", &snapshot.solana_market.condition_id, su, sd, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                        }
                    }
                }
                if enable_xrp {
                    if let (Some(xu), Some(xd)) = (snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref()) {
                        let key = (snapshot.period_timestamp, snapshot.xrp_market.condition_id.clone());
                        if !hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            markets_to_check.push(("XRP", &snapshot.xrp_market.condition_id, xu, xd, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                        }
                    }
                }
                drop(hedge_guard);
            ...
            if DUAL_FILLED_LIMIT_SELL_ENABLED && time_elapsed_seconds >= LIMIT_SELL_AFTER_SECONDS {
            {
                ...
                let mut markets_to_check: Vec<(&str, String, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                ...
                if enable_btc {
                    if let (Some(u), Some(d)) = (snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref()) {
                        ...
                        if !hedge_guard.contains(&(period, cid.clone())) && !dual_guard.contains(&(period, cid.clone())) {
                            markets_to_check.push(("BTC", cid, u, d, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                        }
                    }
                }
                if enable_eth {
                    if let (Some(u), Some(d)) = (snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref()) {
                        ...
                        if !hedge_guard.contains(&(period, cid.clone())) && !dual_guard.contains(&(period, cid.clone())) {
                            markets_to_check.push(("ETH", cid, u, d, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                        }
                    }
                }
                if enable_solana {
                    if let (Some(u), Some(d)) = (snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref()) {
                        ...
                        if !hedge_guard.contains(&(period, cid.clone())) && !dual_guard.contains(&(period, cid.clone())) {
                            markets_to_check.push(("SOL", cid, u, d, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                        }
                    }
                }
                if enable_xrp {
                    if let (Some(u), Some(d)) = (snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref()) {
                        ...
                        if !hedge_guard.contains(&(period, cid.clone())) && !dual_guard.contains(&(period, cid.clone())) {
                            markets_to_check.push(("XRP", cid, u, d, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                        }
                    }
                }
                drop(hedge_guard);
            ...
            if time_elapsed_seconds >= LIMIT_SELL_AFTER_SECONDS {
            {
                ...
                let mut markets_to_check: Vec<(&str, String, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                let mut markets_to_check_under_60: Vec<(&str, String, &crate::models::TokenPrice, &crate::models::TokenPrice, crate::detector::TokenType, crate::detector::TokenType)> = Vec::new();
                    ...
                    if let (Some(u), Some(d)) = (snapshot.btc_market.up_token.as_ref(), snapshot.btc_market.down_token.as_ref()) {
                        ...
                        if hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            if !low_guard.contains(&key) && hedge_price_high(&cid) {
                                markets_to_check.push(("BTC", cid.clone(), u, d, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                            } else if !under_60_guard.contains(&key) && hedge_price_low(&cid) {
                                markets_to_check_under_60.push(("BTC", cid, u, d, crate::detector::TokenType::BtcUp, crate::detector::TokenType::BtcDown));
                            }
                        }
                    }
                }
                if enable_eth {
                    if let (Some(u), Some(d)) = (snapshot.eth_market.up_token.as_ref(), snapshot.eth_market.down_token.as_ref()) {
                        ...
                        if hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            if !low_guard.contains(&key) && hedge_price_high(&cid) {
                                markets_to_check.push(("ETH", cid.clone(), u, d, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                            } else if !under_60_guard.contains(&key) && hedge_price_low(&cid) {
                                markets_to_check_under_60.push(("ETH", cid, u, d, crate::detector::TokenType::EthUp, crate::detector::TokenType::EthDown));
                            }
                        }
                    }
                }
                if enable_solana {
                    if let (Some(u), Some(d)) = (snapshot.solana_market.up_token.as_ref(), snapshot.solana_market.down_token.as_ref()) {
                        ...
                        if hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            if !low_guard.contains(&key) && hedge_price_high(&cid) {
                                markets_to_check.push(("SOL", cid.clone(), u, d, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                            } else if !under_60_guard.contains(&key) && hedge_price_low(&cid) {
                                markets_to_check_under_60.push(("SOL", cid, u, d, crate::detector::TokenType::SolanaUp, crate::detector::TokenType::SolanaDown));
                            }
                        }
                    }
                }
                if enable_xrp {
                    if let (Some(u), Some(d)) = (snapshot.xrp_market.up_token.as_ref(), snapshot.xrp_market.down_token.as_ref()) {
                        ...
                        if hedge_guard.contains(&key) && !two_min_guard.contains(&key) {
                            if !low_guard.contains(&key) && hedge_price_high(&cid) {
                                markets_to_check.push(("XRP", cid.clone(), u, d, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                            } else if !under_60_guard.contains(&key) && hedge_price_low(&cid) {
                                markets_to_check_under_60.push(("XRP", cid, u, d, crate::detector::TokenType::XrpUp, crate::detector::TokenType::XrpDown));
                            }
                        }
                    }
                }
