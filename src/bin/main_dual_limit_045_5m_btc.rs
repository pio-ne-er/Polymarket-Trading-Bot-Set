...
async fn main() -> Result<()> {
    ...
    let binance_btc_usdt = Arc::new(tokio::sync::Mutex::new(None::<f64>));
    let binance_btc_for_monitor = binance_btc_usdt.clone();
    let binance_interval_ms = config.trading.check_interval_ms;
    tokio::spawn(async move {
        ...
        loop {
                    ...
                    if let Ok(json) = resp.json::<serde_json::Value>().await {
                        if let Some(p) = json.get("price").and_then(|v| v.as_str()) {
                            if let Ok(f) = p.parse::<f64>() {
                                *binance_btc_for_monitor.lock().await = Some(f);
                            }
                        }
                    }
                }
            ...
            tokio::time::sleep(tokio::time::Duration::from_millis(binance_interval_ms)).await;
        }
    });
    ...
    let trailing_buy_first_bought: Arc<tokio::sync::Mutex<std::collections::HashMap<(u64, String), (f64, bool)>>> =
        Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::new()));
    // Trailing-buy: only run for markets we saw from the start (new market window). Skip if bot started mid-period.
    let trailing_buy_markets_from_start: Arc<tokio::sync::Mutex<std::collections::HashSet<(u64, String)>>> =
        Arc::new(tokio::sync::Mutex::new(std::collections::HashSet::new()));
    ...
    monitor_arc.start_monitoring(move |snapshot| {
        ...
        let trailing_buy_markets_from_start = trailing_buy_markets_from_start.clone();
        ...
        async move {
            ...
            // Only start trailing for markets we see from the start (within first NEW_MARKET_PLACE_WINDOW_SECONDS). If bot started mid-period, skip until next period.
            if use_trailing_buy_mode {
                ...
                if in_new_market_window {
                    trailing_buy_markets_from_start.lock().await.insert(key.clone());
                }
                if !trailing_buy_markets_from_start.lock().await.contains(&key) {
                    return; // joined mid-period; don't run trailing-buy for this market
                }
                {
                ...
                if let Some((first_price, first_was_up)) = trailing_buy_first_bought.lock().await.get(&key).copied() {
                    // Second trailing: opposite token. band_second = 1 - first_bought_price (target cheap price). Buy as cheap as possible.
                    let (opposite_token, opposite_type, opposite_ask) = if first_was_up {
                        ...
                        (btc_down, crate::detector::TokenType::BtcDown, ask)
                    } else {
                        ...
                        (btc_up, crate::detector::TokenType::BtcUp, ask)
                    };
                    // Time-based band: before 2min = 1-bought-0.1, before 3min = 1-bought, until 4min = 1-bought+0.1.
                    let band_second = if time_elapsed_seconds < 120 {
                        1.0 - first_price - 0.1
                    } else if time_elapsed_seconds < 180 {
                        1.0 - first_price
                    } else {
                        1.0 - first_price + 0.1
                    };
                    ...
                    // Do not buy immediately when below band: trail the price down and buy on bounce (see below).
                    ...
                    if run_4min && opposite_ask >= hedge_price {
                        ...
                        two_min_trailing_min_ask.lock().await.remove(&price_key);
                        *trailing_status_line.lock().await = String::new();
                        let raw_investment = hedge_shares * opposite_ask;
                        let (second_shares, investment) = if raw_investment < 1.0 {
                            (1.0 / opposite_ask, 1.0)
                        } else {
                            (hedge_shares, raw_investment)
                        };
                        let opp = BuyOpportunity {
                            ...
                            dual_limit_shares: Some(second_shares),
                        };
...

                    {
                        ...
                        let mut s = trailing_status_line.lock().await;
                        *s = format!("trail: {} ask={:.2} low={:.2} buy_when<={:.2} (2nd)", opposite_type.display_name(), opposite_ask, trailing_lowest, band_second);
                    }
                    if opposite_ask > band_second {
                        // Above band: update low every snapshot with current market price.
                        two_min_trailing_min_ask.lock().await.insert(price_key.clone(), opposite_ask);
                        return;
                    }
                    // Within band (at or below): update low only if current price is under previous low.
                    ...
                    if opposite_ask < effective_lowest + hedge_trailing_stop {
                        let trigger = effective_lowest + hedge_trailing_stop;
                        *trailing_status_line.lock().await = format!(
                            "trail: {} ask={:.2} low={:.2} buy_when>={:.2} (2nd bounce)",
                            opposite_type.display_name(), opposite_ask, effective_lowest, trigger
                        );
                        crate::log_println!(
                ...
                let (first_token, first_type) = if monitor_up {
                    (btc_up, crate::detector::TokenType::BtcUp)
                } else {
                    (btc_down, crate::detector::TokenType::BtcDown)
                };
            ...
            let (unfilled_token, unfilled_type, current_ask, hedge_shares, filled_side_price) = match (up_trade.as_ref(), down_trade.as_ref()) {
                (Some(up_t), Some(down_t)) if up_t.buy_order_confirmed != down_t.buy_order_confirmed => {
                    if !up_t.buy_order_confirmed && down_t.buy_order_confirmed {
                        let ask = btc_up.ask.map(|a| f64::try_from(a).unwrap_or(0.0)).unwrap_or_else(|| btc_up.bid.map(|b| f64::try_from(b).unwrap_or(0.0)).unwrap_or(0.0));
                        (btc_up, crate::detector::TokenType::BtcUp, ask, down_t.units, down_t.purchase_price)
                    } else {
                        let ask = btc_down.ask.map(|a| f64::try_from(a).unwrap_or(0.0)).unwrap_or_else(|| btc_down.bid.map(|b| f64::try_from(b).unwrap_or(0.0)).unwrap_or(0.0));
                        (btc_down, crate::detector::TokenType::BtcDown, ask, up_t.units, up_t.purchase_price)
                    }
                }
