// Trailing bot: wait for one token under 0.48, then trail that token; trigger only when trigger_at <= 0.48; if price goes above 0.48 without triggering, reset and wait for under 0.48 again. After first buy, monitor opposite with stop loss + trailing stop.
...
/// If we first see a market when elapsed time is within this many seconds of period start, treat as "new market start" and allow trailing when price is already under entry threshold. Otherwise (bot started mid-period) require near 0.5 then cross under entry threshold.
const NEW_MARKET_START_THRESHOLD_SECS: u64 = 60;
...
const TRAILING_ENTRY_MAX_PRICE: f64 = 0.48;
/// Trigger (lowest + trailing_stop_point) above this is ignored; lowest is set to this and trailing continues.
const TRAILING_TRIGGER_CAP: f64 = 0.48;
const STOP_LOSS_BUFFER: f64 = 0.1;
/// After second window (2m30s/4min) from first buy: if first_bought_price + opposite_ask > SUM_HEDGE_THRESHOLD, buy second token immediately.
const SUM_HEDGE_THRESHOLD: f64 = 1.1;
/// Minimum total cost for first buy (USD). If under this, we increase units so cost >= this.
const MIN_FIRST_BUY_COST: f64 = 1.0;
/// Maximum allowed difference between first-token and second-token share counts after hedge; reconciliation buys the short side until diff <= this.
const MAX_HEDGE_BALANCE_DIFF: f64 = 1.0;
...
/// For 15m: first window 120s (2 min), second 240s (4 min). For 5m: first 60s (1 min), second 150s (2m30s).

/// Returns true if opposite_price is at or below the ceiling for the current time-since-first-buy.
...
fn second_token_meets_price_ceiling(
    ...
    seconds_first_window: u64,
    seconds_second_window: u64,
) -> bool {
    let ceiling = if seconds_since_first_buy < seconds_first_window {
        1.0 - first_bought_price - 0.05
    } else if seconds_since_first_buy < seconds_second_window {
...
fn second_token_ceiling_desc(
    ...
    let (ceiling, label) = if seconds_since_first_buy < seconds_first_window {
        (1.0 - first_bought_price - 0.05, format!("within {} of first buy", format_window_secs(seconds_first_window)))
    } else if seconds_since_first_buy < seconds_second_window {
        (1.0 - first_bought_price, format!("within {} of first buy", format_window_secs(seconds_second_window)))
    } else if seconds_since_first_buy >= early_hedge_after_seconds {
        (1.0 - first_bought_price, "after early-hedge min from first buy".to_string())
    } else {
        (1.0 - first_bought_price, format!("{} to early-hedge from first buy", format_window_secs(seconds_second_window)))
    };
    format!("{} requires opposite price <= {:.4}", label, ceiling)
}

fn format_window_secs(secs: u64) -> String {
    if secs == 60 {
        "1min".to_string()
    } else if secs == 90 {
        "1m30s".to_string()
...
fn trailing_window_label(
    ...
    if time_elapsed < seconds_first_window {
        format!("0-{}", format_window_secs(seconds_first_window))
    } else if time_elapsed < seconds_second_window {
        format!("{}-{}", format_window_secs(seconds_first_window), format_window_secs(seconds_second_window))
    } else if time_elapsed < early_hedge_after_seconds {
        format!("{}-early", format_window_secs(seconds_second_window))
    } else if time_elapsed < hedge_after_seconds {
        "early-hedge".to_string()
    } else {
        "post-hedge".to_string()
    }
}
...
#[derive(Debug, Clone)]
enum TrailingState {
    /// We've seen at least one token in [0.48, 0.52] for this market; waiting for one token to go under entry threshold to start trailing.
    WaitingForUnder45,
...
fn ask_f64(token: &crate::models::TokenPrice) -> f64 {
    token
        .ask
        .as_ref()
        .and_then(|d| d.to_f64())
...
/// After second buy: fetch both token balances (with retries for reliability); if difference > MAX_HEDGE_BALANCE_DIFF, buy more of the lower side so diff <= MAX_HEDGE_BALANCE_DIFF. No-op in simulation.
async fn reconcile_hedge_balances(
    trader: &Trader,
    first_token_id: &str,
    second_token_id: &str,
    first_ask: f64,
    ...
    polymarket_arbitrage_bot::log_println!("   ⏳ Reconciliation: checking both balances (2s initial, 3 attempts, 2s apart)...");
    let first_bal = trader
        .get_token_balance_shares_with_retry(first_token_id, 2, 3, 2)
        .await
        .unwrap_or(0.0);
    let second_bal = trader
        .get_token_balance_shares_with_retry(second_token_id, 0, 3, 2)
        .await
        .unwrap_or(0.0);
    polymarket_arbitrage_bot::log_println!(
        "   Balances: first={:.2} second={:.2} (diff {:.2})",
        first_bal,
        second_bal,
        (first_bal - second_bal).abs()
    );
    let diff = (first_bal - second_bal).abs();
    if diff <= MAX_HEDGE_BALANCE_DIFF {
        return;
    }
    let (low_id, low_ask, low_type) = if first_bal < second_bal {
        (first_token_id, first_ask, token_type_for(market_name, first_was_up))
    } else {
        (second_token_id, second_ask, token_type_for(market_name, !first_was_up))
    };
    let max_bal = first_bal.max(second_bal);
    let min_bal = first_bal.min(second_bal);
    let to_buy = ((max_bal - min_bal - MAX_HEDGE_BALANCE_DIFF) * 100.0).ceil() / 100.0;
    if to_buy < 1.0 {
        return;
    }
    let investment = (to_buy * low_ask * 1.03).max(1.0);
    polymarket_arbitrage_bot::log_println!(
        "🔄 Hedge reconciliation: {} first={:.2} second={:.2} (diff {:.2}) → buying {:.2} more of short side",
        market_name, first_bal, second_bal, diff, to_buy
    );
    let opp = BuyOpportunity {
        condition_id: condition_id.to_string(),
        token_id: low_id.to_string(),
        token_type: low_type,
        bid_price: low_ask,
        period_timestamp,
        ...
        dual_limit_shares: Some(to_buy),
    };
    if let Err(e) = trader.execute_buy(&opp).await {
        warn!("Hedge reconciliation buy failed {}: {}", market_name, e);
    } else {
        polymarket_arbitrage_bot::log_println!("   ✅ Reconciliation buy done: +{:.2} shares (target diff ≤ {:.0})", to_buy, MAX_HEDGE_BALANCE_DIFF);
    }
}
...
async fn main() -> Result<()> {
    ...
    let market_minutes = config.trading.trailing_market_minutes.unwrap_or(15);
    let period_seconds = market_minutes * 60;
    let slug_mid = if market_minutes == 5 {
        "-updown-5m-"
    } else {
        "-updown-15m-"
    };
    let period_label = if market_minutes == 5 { "5-minute" } else { "15-minute" };
    eprintln!("Market period: {} ({} min)", period_label, market_minutes);
    // Second-token hedging windows (from first-buy time). Config overrides; else 5m: 1min, 2m30s; 15m: 2min, 4min.
    let default_first = if market_minutes == 5 { 60u64 } else { 120u64 };
    let default_second = if market_minutes == 5 { 150u64 } else { 240u64 };
    let seconds_first_window = config.trading.trailing_first_window_seconds.unwrap_or(default_first);
    let seconds_second_window = config.trading.trailing_second_window_seconds.unwrap_or(default_second);
    eprintln!("Second-token windows: {} then {} (from first buy)", format_window_secs(seconds_first_window), format_window_secs(seconds_second_window));

    let trailing_stop_first = config.trading.trailing_stop_point_first
        .or(config.trading.trailing_stop_point)
        .unwrap_or(0.03);
    let trailing_stop_second = config.trading.trailing_stop_point_second
        .or(config.trading.trailing_stop_point)
        .unwrap_or(0.03);
    let (shares, shares_source) = config
        .trading
        .trailing_shares
        .map(|s| (s, "trailing_shares"))
        .or_else(|| config.trading.dual_limit_shares.map(|s| (s, "dual_limit_shares")))
        .unwrap_or_else(|| (config.trading.fixed_trade_amount / 0.5, "fixed_trade_amount/0.5"));
    eprintln!("Trailing shares: {} (from config: {})", shares, shares_source);

    let trailing_stop_loss_enabled = config.trading.trailing_stop_loss_enabled.unwrap_or(true);
    eprintln!(
        "Strategy: Start trading only when at least one token is near 0.5 ({}–{}). Then wait for token under {:.2}; start trailing. Trigger only when trigger_at <= {:.2}; if trigger_at > {:.2} set lowest = {:.2}. If price goes above {:.2} without trigger, reset and wait again. No match in a market → skip until next {} market. Trailing stop first = {:.4}, second = {:.4}; buy (min cost ${:.0}, base {} units). Opposite: {} or trailing >= opposite_lowest + {:.4}. One hedge per market.",
        NEAR_HALF_MIN, NEAR_HALF_MAX, TRAILING_ENTRY_MAX_PRICE, TRAILING_TRIGGER_CAP, TRAILING_TRIGGER_CAP, TRAILING_TRIGGER_CAP, TRAILING_ENTRY_MAX_PRICE, period_label, trailing_stop_first, trailing_stop_second, MIN_FIRST_BUY_COST, shares,
        if trailing_stop_loss_enabled { format!("stop loss >= (1 - bought_price + {:.2})", STOP_LOSS_BUFFER) } else { "stop loss disabled".to_string() },
        trailing_stop_second
    );
    ...
    if !is_simulation {
        ...
        if let Err(e) = api.authenticate().await {
            warn!("Authentication failed: {} - order placement may fail.", e);
        } else {
    ...
    // When the current period ends, discover new markets and update the monitor so it fetches prices for the new period.
    ...
    let period_seconds_period_check = period_seconds;
    let slug_mid_period_check = slug_mid.to_string();
    let period_label_period_check = period_label.to_string();
    tokio::spawn(async move {
        loop {
            let current_time = std::time::SystemTime::now()
                ...
                .unwrap()
                .as_secs();
            let current_period = (current_time / period_seconds_period_check) * period_seconds_period_check;
            let current_market_timestamp = monitor_for_period_check.get_current_market_timestamp().await;
            let next_period_timestamp = current_market_timestamp + period_seconds_period_check;
            let sleep_duration = if next_period_timestamp > current_time {
                next_period_timestamp - current_time
            } else {
                0
            };
            let sleep_cap = period_seconds_period_check * 2;
            if sleep_duration > 0 && sleep_duration <= sleep_cap {
                tokio::time::sleep(tokio::time::Duration::from_secs(sleep_duration)).await;
            } else if sleep_duration == 0 {
                eprintln!("🔄 New {} period: discovering markets for period {}...", period_label_period_check, current_period);
                match get_or_discover_markets(
                    api_for_period_check.as_ref(),
                    enable_eth_period,
                    enable_solana_period,
                    enable_xrp_period,
                    period_seconds_period_check,
                    &slug_mid_period_check,
                )
                .await
                {
                    Ok((eth_market, btc_market, solana_market, xrp_market)) => {
                            ...
                            .await
                        {
                            ...
                            if let Err(e) = monitor_for_period_check.refresh_token_ids_for_new_period().await {
                                warn!("Trailing bot: failed to refresh token IDs for new period: {}", e);
                            }
    ...
    let period_duration_secs = period_seconds;
    let seconds_first_window = seconds_first_window;
    let seconds_second_window = seconds_second_window;

    let use_websocket = config.trading.trailing_use_websocket.unwrap_or(true);
    eprintln!(
        "Price source: {} (trailing)",
        if use_websocket { "WebSocket" } else { "REST polling" }
    );
    let price_cache: Option<std::sync::Arc<tokio::sync::Mutex<polymarket_arbitrage_bot::monitor::WsPriceCache>>> =
        if use_websocket {
            let cache = Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::new()));
            polymarket_arbitrage_bot::clob_ws::spawn_clob_price_ws_task(cache.clone(), monitor_arc.clone());
            Some(cache)
        } else {
            None
        };
    ...
    monitor_arc
        .start_monitoring_optional_ws(price_cache, move |snapshot| {
            let trader = trader.clone();
            let state = state.clone();
            let history_path = history_path.to_string();
            let trailing_stop_first = trailing_stop_first;
            let trailing_stop_second = trailing_stop_second;
            let shares = shares;
            ...
            let trailing_stop_loss_enabled = trailing_stop_loss_enabled;
            let seconds_first_window = seconds_first_window;
            let seconds_second_window = seconds_second_window;
            ...
            async move {
                ...
                let time_elapsed = period_duration_secs.saturating_sub(snapshot.time_remaining_seconds);
                let current_period_ts = snapshot.period_timestamp;

                // Keep trailing state only for the current period. Drop state for older periods
                // so each new market gets a fresh trailing lifecycle and we don't carry stale entries.
                {
                    let mut guard = state.lock().await;
                    guard.retain(|(p, _), _| *p == current_period_ts);
                }

                let mut markets_full: Vec<(&str, &crate::models::MarketData, u64)> = Vec::new();
                if enable_btc {
                ...
                for (market_name, market, period_ts) in markets_full {
                    ...
                    match s {
                        None => {
                            // Only allow "already under entry threshold" when we're at new market start (within first N sec of period). If bot joined mid-period, require near 0.5 then cross under threshold.
                            let at_market_start = time_elapsed <= NEW_MARKET_START_THRESHOLD_SECS;
                            let up_under = up_ask < TRAILING_ENTRY_MAX_PRICE;
                            let down_under = down_ask < TRAILING_ENTRY_MAX_PRICE;
                            if at_market_start && up_under && !down_under {
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started (new market, already under {:.2}): monitoring Up (ask={:.4})",
                                    market_name,
                                    TRAILING_ENTRY_MAX_PRICE,
                                    up_ask
                                );
                                guard.insert(
                                    key.clone(),
                            ...
                            if at_market_start && down_under && !up_under {
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started (new market, already under {:.2}): monitoring Down (ask={:.4})",
                                    market_name,
                                    TRAILING_ENTRY_MAX_PRICE,
                                    down_ask
                                );
                            ...
                            if at_market_start && up_under && down_under {
                                ...
                                let side = if target_up { "Up" } else { "Down" };
                                let low = if target_up { up_ask } else { down_ask };
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started (new market, already under {:.2}): monitoring {} (ask={:.4})",
                                    market_name,
                                    TRAILING_ENTRY_MAX_PRICE,
                                    side,
                                    low
                            ...
                            // Not at market start, or no token under threshold: only consider when at least one token is near 0.5 (0.48–0.52), then wait for one to go under threshold.
                            ...
                            polymarket_arbitrage_bot::log_println!(
                                "▶ Trailing [{}] market near 0.5 (Up={:.4} Dn={:.4}) → waiting for one token under {:.2}",
                                market_name, up_ask, down_ask, TRAILING_ENTRY_MAX_PRICE
                            );
                        ...
                        Some(TrailingState::WaitingForUnder45) => {
                            // We already saw near 0.5; now start trailing when one token is under threshold.
                            let up_under = up_ask < TRAILING_ENTRY_MAX_PRICE;
                            let down_under = down_ask < TRAILING_ENTRY_MAX_PRICE;
                            if up_under && !down_under {
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started: monitoring Up (ask={:.4}, under {:.2})",
                                    market_name, up_ask, TRAILING_ENTRY_MAX_PRICE
                                );
                                guard.insert(
                                    key.clone(),
                                    TrailingState::MonitoringFirst {
                                ...
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started: monitoring Down (ask={:.4}, under {:.2})",
                                    market_name, down_ask, TRAILING_ENTRY_MAX_PRICE
                                );
                                guard.insert(
                                    key.clone(),
                                    TrailingState::MonitoringFirst {
                                ...
                                let low = if target_up { up_ask } else { down_ask };
                                polymarket_arbitrage_bot::log_println!(
                                    "▶ Trailing [{}] started: monitoring {} (ask={:.4}, both under {:.2})",
                                    market_name, side, low, TRAILING_ENTRY_MAX_PRICE
                                );
                                guard.insert(
                                    key.clone(),
                                    TrailingState::MonitoringFirst {
                        ...
                        Some(TrailingState::MonitoringFirst {
                            ...
                            if *target_is_up {
                                // If price went above threshold without triggering, reset and wait for under threshold again.
                                if up_ask > TRAILING_ENTRY_MAX_PRICE {
                                    polymarket_arbitrage_bot::log_println!(
                                        "🔄 Trailing [{}] Up crossed {:.2} (ask={:.4}) without trigger → reset, wait for token under {:.2} again",
                                        market_name, TRAILING_ENTRY_MAX_PRICE, up_ask, TRAILING_ENTRY_MAX_PRICE
                                    );
                                    guard.remove(&key);
                                    drop(guard);
                                    continue;
                                ...
                                *highest = (*highest).max(up_ask);
                                let trigger_at = *lowest + trailing_stop_first;
                                let window = trailing_window_label(time_elapsed, early_hedge_after_seconds, hedge_after_seconds, seconds_first_window, seconds_second_window);
                                if up_ask < trigger_at {
                                    polymarket_arbitrage_bot::log_println!(
                                        "   Trailing [{}] Up: ask={:.4} lowest={:.4} highest={:.4} trigger_at={:.4} (+{:.3}) [{}]",
                                        market_name, up_ask, *lowest, *highest, trigger_at, trailing_stop_first, window
                                    );
                                    drop(guard);
                                    continue;
                                }
                                // Trigger above cap: ignore, set lowest = cap and continue trailing.
                                if trigger_at > TRAILING_TRIGGER_CAP {
                                    polymarket_arbitrage_bot::log_println!(
                                        "   Trailing [{}] Up: trigger_at {:.4} > {:.2} → ignore, set lowest = {:.2}",
                                        market_name, trigger_at, TRAILING_TRIGGER_CAP, TRAILING_TRIGGER_CAP
                                    );
                                ...
                                if down_ask > TRAILING_ENTRY_MAX_PRICE {
                                    polymarket_arbitrage_bot::log_println!(
                                        "🔄 Trailing [{}] Down crossed {:.2} (ask={:.4}) without trigger → reset, wait for token under {:.2} again",
                                        market_name, TRAILING_ENTRY_MAX_PRICE, down_ask, TRAILING_ENTRY_MAX_PRICE
                                    );
                                    guard.remove(&key);
                                    drop(guard);
                                    continue;
                                ...
                                *highest = (*highest).max(down_ask);
                                ...
                                if down_ask < trigger_at {
                                    polymarket_arbitrage_bot::log_println!(
                                        "   Trailing [{}] Down: ask={:.4} lowest={:.4} highest={:.4} trigger_at={:.4} (+{:.3}) [{}]",
                                        market_name, down_ask, *lowest, *highest, trigger_at, trailing_stop_first, window
                                    );
                                    drop(guard);
                                    continue;
                                }
                                ...
                                if trigger_at > TRAILING_TRIGGER_CAP {
                                    polymarket_arbitrage_bot::log_println!(
                                        "   Trailing [{}] Down: trigger_at {:.4} > {:.2} → ignore, set lowest = {:.2}",
                                        market_name, trigger_at, TRAILING_TRIGGER_CAP, TRAILING_TRIGGER_CAP
                                    );
                                    *lowest = TRAILING_TRIGGER_CAP;
                                    drop(guard);
                        ...
                        Some(TrailingState::FirstBought {
                            ...
                            *opposite_lowest = (*opposite_lowest).min(opposite_price);
                            let trigger_stop_loss = trailing_stop_loss_enabled && (opposite_price >= stop_loss_threshold);
                            let trailing_trigger_at = *opposite_lowest + trailing_stop_second;
                            let trigger_trailing = opposite_price >= trailing_trigger_at;
                            // After 2m30s/4min: if first + opposite > 1.1, buy immediately; else keep trailing.
                            let trigger_sum_over_1_1 = seconds_since_first_buy >= seconds_second_window
                                && (first_price_val + opposite_price) > SUM_HEDGE_THRESHOLD;
                            let side_str = if *first_was_up { "Down" } else { let s = "Up"; s };
                            if !trigger_stop_loss && !trigger_trailing && !trigger_sum_over_1_1 {
                                let sum_str = if seconds_since_first_buy >= seconds_second_window {
                                    format!(" first+opp={:.4} (buy if > {:.1})", first_price_val + opposite_price, SUM_HEDGE_THRESHOLD)
                                } else {
                                    String::new()
                                };
                                polymarket_arbitrage_bot::log_println!(
                                    "   Trailing [{}] opposite {}: price={:.4} opposite_lowest={:.4} stop_loss>= {:.4} trail>= {:.4}{} (waiting)",
                                    market_name, side_str, opposite_price, *opposite_lowest, stop_loss_threshold, trailing_trigger_at, sum_str
                                );
                                drop(guard);
                                continue;
                            }
                            // Stop loss: always buy. Sum over 1.1: buy immediately. Trailing: only buy if opposite is below the time-based ceiling.
                            let allow_buy = if trigger_stop_loss {
                                true
                            } else if trigger_sum_over_1_1 {
                                true
                            } else if trigger_trailing {
                                second_token_meets_price_ceiling(
                                    first_price_val,
                                    seconds_since_first_buy,
                                    opposite_price,
                                    early_hedge_after_seconds,
                                    seconds_first_window,
                                    seconds_second_window,
                                )
                            } else {
                                false
                            };
                            ...
                            let trigger_via = if trigger_stop_loss {
                                "stop_loss"
                            } else if trigger_sum_over_1_1 {
                                "sum_over_1.1"
                            } else {
                            ...
                            if let Err(e) = trader.execute_buy(&opp).await {
                                let err_s = format!("{}", e);
                                let short = if err_s.contains("no orders found to match") || err_s.contains("FAK") {
                                    "FAK: no liquidity"
                                } else if err_s.contains("Insufficient") || err_s.contains("balance") {
                                    "balance/allowance"
                                } else {
                                    err_s.lines().next().unwrap_or("failed").trim()
                                };
                                warn!("Trailing second buy failed {}: {}", market_name, short);
                            } else {
                                ...
                                let via_str = if trigger_stop_loss {
                                    "stop_loss"
                                } else if trigger_sum_over_1_1 {
                                    "sum_over_1.1"
                                } else {
                                ...
                                if !is_simulation {
                                    let (first_tid, second_tid) = if first_was_up_val {
                                        (up_token.token_id.as_str(), down_token.token_id.as_str())
                                    } else {
                                        (down_token.token_id.as_str(), up_token.token_id.as_str())
                                    };
                                    let (first_ask_val, second_ask_val) = if first_was_up_val {
                                        (up_ask, down_ask)
                                    } else {
                                        (down_ask, up_ask)
                                    };
                                    reconcile_hedge_balances(
                                        trader.as_ref(),
                                        first_tid,
                                        second_tid,
                                        first_ask_val,
...
async fn discover_market(
    ...
    let rounded_time = (current_time / period_seconds) * period_seconds;
    for (i, prefix) in slug_prefixes.iter().enumerate() {
        ...
        if include_previous {
            for offset in 1..=3 {
                let try_time = rounded_time - (offset * period_seconds);
...
async fn discover_solana_market(
    ...
    if let Ok(market) = discover_market(api, std::str::from_utf8(&[83,111,108,97,110,97]).unwrap(), &solana_slugs, current_time, seen_ids, false, period_seconds, slug_mid).await {
        return market;
    }
...
async fn discover_xrp_market(
    ...
    if let Ok(market) = discover_market(api, std::str::from_utf8(&[88, 82, 80]).unwrap(), &xrp_slugs, current_time, seen_ids, false, period_seconds, slug_mid).await {
        return market;
    }
    disabled_xrp_market()
}
...
async fn get_or_discover_markets(
    ...
    let eth_market = if enable_eth {
        discover_market(api, std::str::from_utf8(&[69,84,72]).unwrap(), &[std::str::from_utf8(&[101,116,104]).unwrap()], current_time, &mut seen_ids, true, period_seconds, slug_mid).await
    ...
    let btc_market = discover_market(api, btc_name, &btc_slugs, current_time, &mut seen_ids, true, period_seconds, slug_mid).await
        .unwrap_or_else(|_| {
            crate::models::Market {
    ...
    let solana_market = if enable_solana {
        discover_solana_market(api, current_time, &mut seen_ids, period_seconds, slug_mid).await
    } else {
        disabled_solana_market()
    };
    let xrp_market = if enable_xrp {
        discover_xrp_market(api, current_time, &mut seen_ids, period_seconds, slug_mid).await
    } else {
        disabled_xrp_market()
    };
