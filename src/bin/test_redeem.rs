...
async fn main() -> Result<()> {
            ...
            match tokens_result {
                Ok(tokens) => {
                    if tokens.is_empty() {
                        ...
                        println!("\n💡 Tips:");
                        println!("   - This scan checks BTC 5-minute markets (btc-updown-5m-*) from the last ~6 hours");
    ...
    match all_tokens.iter().find(|(tid, _, _, _)| tid == token_id) {
        ...
        None => {
            anyhow::bail!("Token not found in portfolio. Make sure you own this token and it's from a BTC 5-minute market.");
        }
    }
}
